...
P_LIST            = [10]   # this ablation only uses P_parent=10
...
# ── (superseded header kept for history) ────────────────────────
# unique tags that cannot collide with existing (_ga0p1) runs.
ABLATION_CSV = "summary_retrain_sync_scope_ablation_p10_v1.csv"
# ── the parent-state sync-scope ablation conditions (serial order) ──
# guidance = retained_preactivation / weighting = grad_auto alpha=0.1 fixed
# (baseline sync=none is the existing _distretpre_ga0p1 run — not re-run).
# weighting is an independent axis, later swappable to alpha=1.0 / coord_equal_fixed.
SYNC_INTERVAL = 128
# 6 conditions (user-specified order). Two axes: guidance x forcing scope:
#   guidance : retained / full / none (= no auxiliary loss; mode stays
#              retained with lambda=0, auto OFF -> L = L_out only; sync code and teacher shared)
#   forcing  : readout / readout_retained / full / none
# the teacher (L_out target, sync source, init) is the parent's autonomous cache in every condition.
# baseline (guidance x no forcing) = the existing _distretpre/_distfullpre
# _ga0p1 runs; not re-run.
# execution order (changed to the user-specified priority, 2026-09-20):
#   full+fullstate -> symbol+readout -> noguide+readout(rest) ->
#   full+readout -> autonomous noguide -> (completed retained cells skip at the end)
ABLATION_CONDITIONS = [
    dict(name="full_sync_fullstate_ga0p1_v1",
         mode="full_preactivation", weighting="default",
         auto=True, alpha=0.1, sync_scope="full",
         lam_rule="lambda = 0.1 * g_out / (g_aux + 1e-12)"),
    # symbol guidance + readout forcing (parent teacher). The completed
    # symbol runs (_distsym_ga0p1) had no TF (disabled via n_interleave=L+1),
    # so this cell adds parent-state forcing on the readout; weighting is
    # identical to the existing symbol runs (grad_auto alpha=0.1).
    dict(name="symbol_sync_readout_ga0p1_v1",
         mode="symbol", weighting="default",
         auto=True, alpha=0.1, sync_scope="readout",
         lam_rule="lambda = 0.1 * g_out / (g_aux + 1e-12)"),
    dict(name="noguide_sync_readout_v1",
         mode="retained_preactivation", weighting="default",
         auto=False, alpha=None, sync_scope="readout", lam_manual=0.0,
         lam_rule="lambda = 0 (no guidance, L_out only)"),
    dict(name="full_sync_readout_ga0p1_v1",
         mode="full_preactivation", weighting="default",
         auto=True, alpha=0.1, sync_scope="readout",
         lam_rule="lambda = 0.1 * g_out / (g_aux + 1e-12)"),
    # no guidance + no forcing (parent-weight init + relu_mask still used;
    # teacher is the parent free-run L_out only; fully autonomous 200 steps).
    # Isolates the difficulty of the autonomous distillation setting itself.
    dict(name="noguide_nosync_v1",
         mode="retained_preactivation", weighting="default",
         auto=False, alpha=None, sync_scope="none", lam_manual=0.0,
         lam_rule="lambda = 0 (no guidance, no forcing)"),
    # the following 2 conditions are complete (139/141, 138/141) — pass in minutes via checkpoint skip
    dict(name="retained_sync_readout_retained_ga0p1_v1",
         mode="retained_preactivation", weighting="default",
         auto=True, alpha=0.1, sync_scope="readout_retained",
         lam_rule="lambda = 0.1 * g_out / (g_aux + 1e-12)"),
    dict(name="retained_sync_readout_ga0p1_v1",
         mode="retained_preactivation", weighting="default",
         auto=True, alpha=0.1, sync_scope="readout",
         lam_rule="lambda = 0.1 * g_out / (g_aux + 1e-12)"),
]
# 'direct training without forcing' (scratch training with no parent at all)
# is handled by run_direct_p3_noforcing.py, not this retrain launcher.
    ...
    if RETRAIN_REDUCED:
        ...
        cmd += ["--retrain_reduced",
                ...
                *[",".join(map(str, it['run_ids'])) for it in items],
                ...
                "--distill_lambda_full",
                str(CURRENT_COND.get("lam_manual", DISTILL_LAMBDA_FULL)),
                "--distill_lambda_retained",
                str(CURRENT_COND.get("lam_manual", DISTILL_LAMBDA_RETAINED)),
...
def ablation_preflight() -> None:
    """Collision check + sync-scope ablation config table + representative candidates."""
    import glob as _glob
    print("=" * 72)
    print("GUIDANCE x PARENT-STATE FORCING ABLATION (P_parent=10)")
    print("baseline (retained/full guidance, no forcing) = the existing "
          "_distretpre_ga0p1 / _distfullpre_ga0p1 runs (not re-run)")
    print("=" * 72)
    # the collision check allows this ablation's own resume:
    # condition names carry unique _v1 suffixes, so existing models under
    # the same name are this ablation's completed runs (checkpoint-skip).
    # When present, report resume instead of aborting.
    ...
    hdr = (f"{'condition':>42} {'guidance':>10} {'forcing':>17} "
           f"{'intv':>5}  lambda rule")
    print(hdr); print("-" * len(hdr))
    _gd = {'retained_preactivation': 'retained',
           'full_preactivation': 'full',
           'symbol': 'symbol'}
    for c in ABLATION_CONDITIONS:
        guide = ('none' if c.get('lam_manual') == 0.0
                 else _gd[c['mode']])
        print(f"{c['name']:>42} {guide:>10} {c['sync_scope']:>17} "
              f"{SYNC_INTERVAL:>5}  {c['lam_rule']}")
    print("sync source        = parent_autonomous_cache "
          "(raw data is used neither as target nor for sync)")
    print("segment-start sync = full parent state (all conditions, as before)")
    ...
    if sel_files:
        ...
        print(f"\nselected candidates: {len(cands)} "
              f"(same candidate set as the existing P=10 retained runs)")
        c0 = cands[0]
        pe = int(c0.get("p_effective", c0.get("P_effective")))
        deleted = c0.get("deleted_relu_local_indices", [])
        R = [20 - 10 + j for j in range(10) if j not in deleted]
        print(f"representative: seed{c0.get('source_seed', c0.get('seed'))} "
              f"cand{c0['candidate_id']}: N=3, M=20, P_eff={pe}, R={R}")
        print(f"  synced coords: readout=3, readout_retained={3 + pe}, "
              f"full=20")
    print("=" * 72)
    ...
    for n_interleave in N_INTERLEAVE_LIST:
        for ratio_cfg in conditions:
            ...
            if RETRAIN_REDUCED:
                ...
                # later modes HIT it.
                # run the conditions serially in ABLATION_CONDITIONS order
                ...
                for blk, _cond in enumerate(ABLATION_CONDITIONS, 1):
                    ...
                    print(f"#   mode={_cond['mode']}  "
                          f"sync_scope={_cond['sync_scope']}  "
                          f"interval={SYNC_INTERVAL}  "
                          f"weighting=grad_auto  alpha={_cond['alpha']}")
                    print(f"{'#' * 60}")
