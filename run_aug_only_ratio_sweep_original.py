...
# The reverse side covers [15, 14, 17]. P=16 is excluded from both because
# seed10's reduction spec is missing (it would stop with FileNotFoundError).
P_LIST            = [10, 11, 12, 13, 14, 15]
...
#   all candidates; do NOT simply run every per-source local minimum.
# TARGET_P_EFFS:
...
# NOTE: the global minimum itself is always determined from the full scan of
#   all candidates; TARGET_P_EFFS is only a post-filter on what to retrain.
...
#  runtime; it is also the default that make_retrain_summary_table.py reads.)
# DISTILL_MODES = ["none", "full_preactivation", "retained_preactivation", "symbol"]
DISTILL_MODES = ["none", "full_preactivation"]
...
# thread each (OMP_NUM_THREADS=1 below; BLAS parallelism is a net loss for
# the tiny M=20 model), so process count = cores used; match physical cores.
...
def retrained_model_path(tag: str, spec: dict, candidate: dict,
    ...
    return Path("models") / (
        ...
        f"{DISTILL_TAGS[DISTILL_MODE]}{distill_autolambda_tag()}"
...
def make_log_path(ratio_name: str, n_interleave: int, seed: int,
    ...
    if RETRAIN_REDUCED:
        return (LOG_DIR / f"{ratio_name}_retrain__nint{n_interleave}"
                          ...
                          f"{DISTILL_TAGS[DISTILL_MODE]}"
                          f"{distill_autolambda_tag()}.log")
...
def spawn_one(ratio_cfg: dict, n_interleave: int, seed: int,
    ...
    RETRAIN_REDUCED=True: one call = one (source seed, candidate, retrain seed).
      ...
      (plan_retrain_candidates); this receives a single candidate to run
...
def run_retrain_condition(ratio_cfg: dict, n_interleave: int,
    ...
    def _one_run(seed, retrain_seed, it, cid):
        ...
        try:
            ...
            # counts as incomplete and is retried automatically by the skip
            with print_lock:
    ...
    def _run_seed_waves(seed, retrain_seed, runs):
        for i in range(0, len(runs), MAX_PARALLEL_CANDIDATES):
            ...
            # each run in the wave does acquire->spawn->wait->release in its own
            # thread; the next wave starts after all finish (wave order kept)
    ...
    for n_interleave in N_INTERLEAVE_LIST:
        for ratio_cfg in conditions:
            ...
            if RETRAIN_REDUCED:
                ...
                # later modes HIT it.
                n_blocks = len(_orig_p_list) * len(DISTILL_MODES)
                blk = 0
                for _p in _orig_p_list:
                    for _mode in DISTILL_MODES:
                        blk += 1
                        P_LIST = [_p]
                        DISTILL_MODE = _mode
                        print(f"\n{'#' * 60}")
                        print(f"# block {blk}/{n_blocks}:  P_original={_p}  "
                              f"DISTILL_MODE={_mode}")
                        print(f"{'#' * 60}")
